...
export default function HackerRootsTransition({ duration = 3000, loginBoxRect, isAuthorized = false, rootsPulse = false }) {
  const canvasRef = useRef(null);

  useEffect(() => {
    if (!loginBoxRect) return;
      ...
      for (let r = 0; r < rootsConfig.length; r++) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          if (isAuthorized && pulse) {
            let t = r / 10;
            pulseScale = 1 + 0.19 * Math.exp(-t) * Math.sin(t * 11);
          }
          ...
          ctx.shadowBlur = isAuthorized ? 12 : 6;
